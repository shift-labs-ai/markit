...
fn split_y_lines_into_groups(
    ...
    for i in 1..y_lines.len() {
        ...
        let cols = count_bridging_v_line_cols(upper_y, lower_y, verticals);
        if cols == 0 {
...
fn expand_sub_rows_by_y_clusters(
    original_rows: usize,
    cols: usize,
    ...
    for orig_row in 0..original_rows {
        ...
        // We need to update cell_boxes keys for shifted cells
        let mut new_cell_boxes: HashMap<usize, Vec<usize>> = HashMap::new();
        for (&k, v) in cell_boxes.iter() {
            new_cell_boxes.insert(k, v.clone());
        }
        // Actually, since cell indices might change when we add new cells,
        // we need a different approach. Let's use (row, col) as key temporarily.

        // Actually the TS code uses TableCell references as Map keys, but in Rust
        // we use indices. Since we're adding cells and shifting rows, we need to
        // be careful. Let me rebuild cell_boxes by (row, col) pairs.
