...
mod js_sort_tests {
    ...
    fn matches_bun_jsc_sort_on_intransitive_cmp() {
        // Committed fixtures generated by Bun's Array#sort with this exact
        // tolerance-band comparator. This parity gate must never silently skip.
        let cases: serde_json::Value =
            serde_json::from_str(include_str!("../../../testdata/pdf/js_sort.json"))
                .expect("committed JSC sort fixture must be valid JSON");
        for (ci, case) in cases.as_array().unwrap().iter().enumerate() {
            ...
            assert_eq!(input, expected, "fuzz case {ci}");
        }
    }
