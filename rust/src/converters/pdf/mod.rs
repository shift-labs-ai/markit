...
fn gallop_left<T>(key: &T, base: &[T], length: usize, hint: usize, lt: &impl Fn(&T, &T) -> bool) -> usize {
    ...
    if lt(&base[hint], key) {
        ...
        while offset < max_offset {
            ...
            offset = if next_offset > offset { next_offset.min(max_offset) } else { max_offset };
        }
        last_offset += hint;
        offset += hint;
    } else {
...
fn gallop_right<T>(key: &T, base: &[T], length: usize, hint: usize, lt: &impl Fn(&T, &T) -> bool) -> usize {
    debug_assert!(hint < length);
    let mut last_offset = 0usize;
    let mut offset = 1usize;
...
fn merge_powersort_runs<T: Clone>(
    ...
    let skip_left = gallop_right(&src[src_index2], &src[src_index1..src_end1], left_length, 0, lt);
    if skip_left > 0 {
    ...
    if skip_right > 0 {
        dst[src_end2 - skip_right..src_end2].clone_from_slice(&src[src_end2 - skip_right..src_end2]);
    }
...
fn array_stable_sort<T: Clone>(src: &mut [T], lt: &impl Fn(&T, &T) -> bool) {
    ...
    while run1.end + 1 < num_elements {
        let mut run2 = SortedRun { begin: run1.end + 1, end: run1.end + 1 };
...
mod js_sort_tests {
    ...
    fn intransitive_band_cmp_is_deterministic() {
        let boxes = vec![
            (10.0, 5.0), (9.0, 1.0), (8.5, 3.0), (7.0, 2.0), (10.5, 0.5),
            (9.5, 9.0), (8.0, 4.0), (11.0, 1.5),
        ];
